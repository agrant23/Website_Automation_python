...
class HomePageSetup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.driver = webdriver.Chrome(Settings.path_to_webdriver,
                                       options=options)  
...
class Search(HomePageSetup):
    """
    Confirm the cursor is present in the Search field after the yahoo home page
    is opened. After an arbitrary input is given to the search field confirm
    that clicking the search button navigates the user to the search page.
...
class Dynamic_Drop_Down(HomePageSetup):
    """
    For the Originals drop down menu within yahoo news; ensure that clicking on
    this drop down's options sends the user to the correlating site.

    acceptance criteria   
    --------------------
    -The title of the Originals drop down options sends the user to the given
     by the provided dictionary.
    """
    ...
    def test_any_random_tab(self):  
                                                                                #Dictionary of option title as the key and the corresponding url block as the value. This is set up for many different tabs, as this was the case previously.
...
class Log_In_Link(HomePageSetup):
    ...
    def test_log_in_link(self):
        ...
        acceptance criteria   
        --------------------
        -After clicking the sign in button, the URL has changed to the login
         page. 
        """
...
class Error_Message_Passwords(HomePageSetup):
    ...
    def test_short_password_error_message(self):
        """ 
        Confirm that when the password is shorter than the minimum length
        required, the error data is present.
        ...
        self.assertEqual(yahoo_page.password_error_message().get_attribute(
    ...
    def test_long_password_error_message(self):
        """ 
        Confirm that when the password longer than the maximum length allowed,
        the error data is not present. Confirm there is no error text after
        entering a password of sufficient length.
        ...
        self.assertEqual(yahoo_page.password_error_message().get_attribute(
            'data-error'),"")
        self.assertTrue(yahoo_page.password_error_message().text=="" or
            yahoo_page.password_error_message().text==None, "\n" +
            yahoo_page.password_error_message().text +
            ' appeared instead of an empty string or None')
        #The assertTrue above is necessary due to the error messaging returning
        #both an empty string and None. Using an AssertEqual does not
        #work with logic operators
